...
function isBackdatingAllowed(startDate) {
  const windowDays = Config.backdatingWindowDays();
  const earliest = dayjs().subtract(windowDays, 'day');
  const leaveYearStart = dayjs(`${dayjs().year()}-${Config.get('leave_year_start', '04-01')}`);
  const effectiveEarliest = earliest.isAfter(leaveYearStart) ? earliest : leaveYearStart;
  return !dayjs(startDate).isBefore(effectiveEarliest, 'day');
}
